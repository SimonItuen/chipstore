...
class ClearanceProduct extends StatelessWidget {
  final String thumbnailUrl;
  final String tag;
  final Color color;
  final bool hasTags;
  final VoidCallback onAddCartPressed;
  const ClearanceProduct({Key? key, required this.thumbnailUrl, required this.tag, required this.color, this.hasTags=true, required this.onAddCartPressed}) : super(key: key);
      ...
      child: Stack(
        children: [
          Positioned(
            child: Column(
              ...
              children: [
                ...
                Padding(padding: EdgeInsets.all(2.0.toScreenWidth(context))),
                Text('KD 4.750', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Theme.of(context).primaryColor),),
                Padding(padding: EdgeInsets.all(4.0.toScreenWidth(context))),
                InkWell(
                ...
                Padding(
                  padding: EdgeInsets.only(top: 12.0.toScreenWidth(context), bottom: 32.0.toScreenWidth(context)),
                  child: Center(child: Text('Check in-store stock', style: TextStyle(fontSize: 14, color: Theme.of(context).primaryColor),)),
                ),
              ],
            ),
          ),
          Positioned(
            top: 8,
              right: 8,
              child:  Icon(ChipstoreAppIcons.favorite, color: Theme.of(context).primaryColor, size: 14,)),
          Visibility(
            visible: hasTags,
            child: Positioned(
              top: 8,
